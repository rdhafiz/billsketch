...
class Login
{
    ...
    public static function make($request): array
    {
        try {
            ...
            // Checking if the user's account needs activation
            if ($userInfo->activation_code != null) {
                // Sending a verification email and prompting the user to verify the account
                AuthRepository::sendVerificationEmail($userInfo);
                return ['status' => 300, 'message' => 'Your account is not verified yet. We sent a mail again. Please verify your account'];
            } else {
                ...
                if (Hash::check($requestData['password'], $userInfo->password)) {
                    ...
                    // Logging the user's login activity
