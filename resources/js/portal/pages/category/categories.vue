        ...
        <div class="card-body">
            <div class="row mb-3 align-items-center">
                ...
                <div class="col-lg-4 col-xxl-7 text-end">
                    <router-link :to="{name: 'CategoryCreate'}" class="btn btn-theme" style="width: 120px;">Create
                    </router-link>
                </div>
            </div>
                ...
                <table class="table table-hover">
                    ...
                    <tr v-for="(each, index) in tableData">
                        ...
                        <td class="text-end" style="min-width: 180px;">
                            <router-link :to="{name: 'CategoryEdit', params: {id: each.id}}" class="btn btn-theme">
                                <i class="fa fa-pencil" aria-hidden="true"></i>
                            </router-link>
                            <button class="btn btn-secondary ms-2" @click="updateCategoryStatus(each.id)">
                            ...
                            <button class="btn btn-danger ms-2" @click="deleteCategory(each.id)">
...
export default {
    ...
    methods: {
        ...
        deleteCategory(id) {
            swal({
                title: "Are you sure?",
                text: "If you delete a category, the system will unassign that category from all associated invoices. The invoices themselves remain intact.",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                .then(willDelete => {
                    console.log(1)
                    if (willDelete) {
                        apiService.POST(apiRoutes.categoryDelete, {id}, (res) => {
                            if (res.status === 200) {
        ...
        updateCategoryStatus(id) {
            swal({
                title: "Are you sure?",
                text: `Are you sure that you want to ${this.status === '' ? 'archive' : 'restore'} this category?`,
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
                .then(willDelete => {
                    if (willDelete) {
                        apiService.POST(apiRoutes.categoryStatus, {id}, (res) => {
                            if (res.status === 200) {
                                swal(`${!this.status ? 'Archived!' : 'Restored!'}`, `${!this.status ? 'Category has been archived!' : 'Category has been restored!!'}`, "success"
                                );
                                this.getCategories();
                            } else {
                                swal("Error!", res.errors?.id[0], "error");
